...
{
    public static class DapperGeneratorOracle
    {
        const string ConnectionStringPlaceholder = "\"<ConnectionString>\"";

        public static string GenerateDapperFromDatabase(Database database, string defaultNamespace = "ICB.MOMS.API.Models.Gateway")
        {
        ...
        private static void GenerateDapperGetMethodsFromTable(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
        ...
        {
            stringBuilder.AppendLine($"{PadBy(2)}public async Task<IEnumerable<{table.DataModelName}>> GetAll{table.TableName}()");
            stringBuilder.AppendLine($"{PadBy(2)}{{");
            stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
            stringBuilder.AppendLine($"{PadBy(3)}{{");
            stringBuilder.AppendLine($"{PadBy(4)}const string getAllQuery = \"SELECT * FROM {table.TableName}\";");
            stringBuilder.AppendLine($"{PadBy(4)}var results = await db.QueryAsync<{table.DataModelName}>(getAllQuery);");
            ...
            stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
            stringBuilder.AppendLine($"{PadBy(3)}{{");
            stringBuilder.AppendLine($"{PadBy(4)}const string getByAllQuery = \"SELECT * FROM {table.TableName} WHERE {sqlWhereClauses}\";");
            stringBuilder.AppendLine($"{PadBy(4)}var results = await db.QueryAsync<{table.DataModelName}>(getByAllQuery, new {{ {dapperProperties} }});");
            stringBuilder.AppendLine($"{PadBy(4)}return results;");
            stringBuilder.AppendLine($"{PadBy(3)}}}");
            stringBuilder.AppendLine($"{PadBy(2)}}}");
            ...
            stringBuilder.AppendLine($"{PadBy(4)}const string getByAnyQuery = \"SELECT * FROM {table.TableName} WHERE {sqlWhereClauses.Replace(" AND ", " OR ").Replace("IS NULL OR", "IS NOT NULL AND")}\";");
            stringBuilder.AppendLine($"{PadBy(4)}var results = await db.QueryAsync<{table.DataModelName}>(getByAnyQuery, new {{ {dapperProperties} }});");
            stringBuilder.AppendLine($"{PadBy(4)}return results;");
            stringBuilder.AppendLine($"{PadBy(3)}}}");
            stringBuilder.AppendLine($"{PadBy(2)}}}");
        }
        ...
        private static void GenerateDapperInsertMethodsFromTable(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            stringBuilder.AppendLine($"{PadBy(2)}public async Task<int> Insert({methodParameters})");
            stringBuilder.AppendLine($"{PadBy(2)}{{");
            stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
            stringBuilder.AppendLine($"{PadBy(3)}{{");
            stringBuilder.AppendLine($"{PadBy(4)}const string insertQuery = \"INSERT INTO {table.TableName} ({columnNames}) VALUES ({sqlInsertValues})\"; RETURNING ID INTO :NewId");
        ...
        private static void GenerateDapperUpdateMethodsFromTable(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(2)}public async Task<int> Update({methodParameters})");
                stringBuilder.AppendLine($"{PadBy(2)}{{");
                stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
                stringBuilder.AppendLine($"{PadBy(3)}{{");
        ...
        private static void GenerateDapperDeleteMethodsFromTable(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                var sqlWhereClausesForPrimaryKeys = primaryKeyColumns.GetSqlWhereClauses(":",false);
                var dapperParametersForPrimaryKeys = primaryKeyColumns.GetDapperProperties();

                stringBuilder.AppendLine($"{PadBy(2)}public async Task<int> Delete({methodParameters})");
                stringBuilder.AppendLine($"{PadBy(2)}{{");
                stringBuilder.AppendLine($"{PadBy(3)}using (IDbConnection db = new OracleConnection({ConnectionStringPlaceholder}))");
                stringBuilder.AppendLine($"{PadBy(3)}{{");
